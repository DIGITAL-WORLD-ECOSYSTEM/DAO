...
import { Form , toast, Field, FormHead, schemaUtils, FormReturnLink } from 'src/identity/components';
...
export function CenteredResetPasswordView() {
  ...
  const onSubmit = handleSubmit(async (data: ResetPasswordSchemaType) => {
    try {
      ...
      await axios.post(endpoints.auth.resetPassword || '/api/core/identity/reset-password', {
        email: data.email,
      });
      ...
      router.push(paths.identity.updatePassword);
  ...
  return (
    <>
      ...
      <FormReturnLink href={paths.identity.signIn} />
    </>
  );
}