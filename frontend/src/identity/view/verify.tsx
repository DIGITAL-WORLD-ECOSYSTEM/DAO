...
import { toast } from 'src/identity/components';
...
export function CenteredVerifyView() {
  ...
  const onSubmit = handleSubmit(async (data: VerifySchemaType) => {
    try {
      ...
      await axios.post(endpoints.auth.verify || '/api/core/identity/verify', {
        email: data.email,
        code: data.code,
      });

      toast.success('E-mail verificado com sucesso!');
      router.push(paths.identity.signIn);
  ...
  const handleResendCode = async () => {
    try {
      ...
      await axios.post('/api/core/identity/resend-code', { email });
