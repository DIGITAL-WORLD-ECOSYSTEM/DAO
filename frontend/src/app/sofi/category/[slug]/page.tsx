...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const categoryName = _sofi.find((p) => kebabCase(p.category) === slug)?.category || slug;
  ...
  return {
    ...
    openGraph: {
      ...
      url: `${CONFIG.siteUrl}/sofi/category/${slug}`,
    },
  };
}
 ...
 * Filtra e renderiza a lista de sofi pertencentes a uma categoria específica.
 */
export default async function Page({ params }: Props) {
  const { slug } = await params;

  // 🔍 FILTRAGEM: Busca sofi cujo kebabCase da categoria coincida com o slug da URL
  const filteredSoFis = _sofi.filter((post) => kebabCase(post.category) === slug);

  // Caso não existam sofi para o slug fornecido, dispara a página 404
  if (filteredSoFis.length === 0) {
    notFound();
  }
   ...
   * O 'JSON.stringify/parse' limpa o objeto filteredSoFis de qualquer método 
   * ou propriedade não-serializável que venha do arquivo de Mocks.
   */
  const sanitizedSoFis = JSON.parse(JSON.stringify(filteredSoFis));

  return <SoFiListView sofi={sanitizedSoFis} />;
}
