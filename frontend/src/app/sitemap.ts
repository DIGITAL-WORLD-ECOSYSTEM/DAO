...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticRoutes: MetadataRoute.Sitemap = [
    ...
    },
    {
      ...
      lastModified: new Date().toISOString(),
      changeFrequency: 'daily', // Aumentado para diário para acelerar indexação de novos sofi
      priority: 0.8,
    },
  ];

  // 2. Rotas Dinâmicas: SoFis e Ativos RWA
  const postRoutes: MetadataRoute.Sitemap = _sofi.map((post) => ({
  ...
  const categories = [...new Set(_sofi.map((post) => post.category))];
