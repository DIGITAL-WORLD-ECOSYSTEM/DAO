...
import type { IUserProfile, IUserProfileSoFi } from 'src/types/user';
...
import { ProfileSoFiItem } from './profile-post-item';

// ----------------------------------------------------------------------

type Props = GridProps & {
  info: IUserProfile;
  sofi: IUserProfileSoFi[];
};

export function ProfileHome({ info, sofi, sx, ...other }: Props) {
  ...
  const renderSoFiInput = () => (
    <Card sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Button variant="contained">SoFi</Button>
      </Box>
  ...
  return (
    <Grid container spacing={3} sx={sx} {...other}>
      ...
      <Grid size={{ xs: 12, md: 8 }} sx={{ gap: 3, display: 'flex', flexDirection: 'column' }}>
        ...
        {sofi.map((post) => (
          <ProfileSoFiItem key={post.id} post={post} />
        ))}
      </Grid>
    </Grid>
  );
