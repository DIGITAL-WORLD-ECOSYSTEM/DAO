...
export function SoFiFeatured({ sx }: { sx?: SxProps<Theme> }) {
  ...
  return (
    <Box sx={{ position: 'relative', overflow: 'hidden', bgcolor: 'transparent', ...sx }}>
      <Carousel carousel={carousel}>
        {staticFeaturedSoFis.map((post) => (
          <SoFiItem key={post.id} post={post} />
        ))}
      </Carousel>
...
function SoFiItem({ post }: { post: any }) {
  const theme = useTheme();
