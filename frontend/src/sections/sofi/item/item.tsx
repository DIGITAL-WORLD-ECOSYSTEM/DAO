...
type SoFiItemProps = CardProps & {
  post: ISoFiItem;
  detailsHref: string;
};

export function SoFiItem({ post, detailsHref, sx, ...other }: SoFiItemProps) {
...
type SoFiItemLatestProps = {
  post: ISoFiItem;
  index: number;
  detailsHref: string;
};

export function SoFiItemLatest({ post, index, detailsHref }: SoFiItemLatestProps) {
  const theme = useTheme();
...
type InfoBlockProps = BoxProps & Pick<ISoFiItem, 'totalViews' | 'totalShares' | 'totalComments'>;
