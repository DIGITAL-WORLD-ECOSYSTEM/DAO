...
import { SoFiItem, SoFiItemLatest } from './item';

// ----------------------------------------------------------------------

const staticMeioAmbienteSoFis = [
  {
    id: 'm1',
...
export function MeioAmbiente() {
  ...
  const viewSoFis = staticMeioAmbienteSoFis;
    ...
    >
      <Container component={MotionViewport}>
        ...
        <Grid container spacing={4}>
          {/* Desktop: Destaques (SoFis 1-3) */}
          {viewSoFis.slice(0, 3).map((post, index) => (
            <Grid
          ...
          {/* Mobile/Tablet: Destaques (SoFis 1-3) */}
          {viewSoFis.slice(0, 3).map((post, index) => (
            <Grid
            ...
            >
              <m.div variants={varFade('inUp')}>
                <Box sx={cardWrapperStyle}>
                  <SoFiItem post={post as any} detailsHref={paths.post.details(post.title)} />
                </Box>
              </m.div>
            </Grid>
          ))}

          {/* Lista Restante (SoFis 4-7) */}
          {viewSoFis.slice(3, 7).map((post, index) => (
            <Grid
