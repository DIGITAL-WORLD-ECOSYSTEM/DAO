...
export function SoFiRecent({ sofi: sofiFromProps }: Props) {
  ...
  // ✅ Prioriza os sofi da API e usa estáticos como fallback
  const sofi = (sofiFromProps && sofiFromProps.length > 0) 
    ? sofiFromProps 
    : staticRecentSoFis;
    ...
    >
      <Container component={MotionViewport}>
        <Grid container spacing={4}>
          ...
          {sofi.slice(0, viewLimit).map((post) => (
