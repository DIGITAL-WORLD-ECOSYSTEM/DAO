...
export function Economia() {
  ...
  const viewSoFis = staticEconomiaSoFis;
    ...
    >
      <Container component={MotionViewport}>
        ...
        <Grid container spacing={4}>
          ...
          {viewSoFis.slice(0, 3).map((post: any, index: number) => (
            <Grid
          ...
          {viewSoFis.slice(0, 3).map((post: any, index: number) => (
            ...
            >
              <m.div variants={varFade('inUp')}>
                <Box sx={cardStyle}>
                  <SoFiItem post={post} detailsHref={paths.post.details(post.title)} />
                </Box>
              </m.div>
            </Grid>
          ))}

          {/* Lista Restante (SoFis 4 a 7) */}
          {viewSoFis.slice(3, 7).map((post: any, index: number) => (
            <Grid
