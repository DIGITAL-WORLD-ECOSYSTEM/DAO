 ...
 * Role: SoFi Create/Edit Form (Admin Management)
...
import { SoFiDetailsPreview } from './post-details-preview';

// ----------------------------------------------------------------------

/**
 * 🛡️ SCHEMA DE VALIDAÇÃO (ZOD):
 * Define as regras de negócio para a criação de sofi.
 * 'schemaUtils' é utilizado para validar inputs complexos como Editor e Upload.
 */
export const SoFiCreateSchema = z.object({
...
export type SoFiCreateSchemaType = z.infer<typeof SoFiCreateSchema>;

type Props = {
  currentSoFi?: ISoFiItem;
};

// ----------------------------------------------------------------------

export function SoFiCreateEditForm({ currentSoFi }: Props) {
   ...
   */
  const defaultValues = useMemo<SoFiCreateSchemaType>(() => ({
    title: currentSoFi?.title || '',
    description: currentSoFi?.description || '',
    content: currentSoFi?.content || '',
    coverUrl: currentSoFi?.coverUrl || null,
    tags: currentSoFi?.tags || [],
    metaKeywords: currentSoFi?.metaKeywords || [],
    metaTitle: currentSoFi?.metaTitle || '',
    metaDescription: currentSoFi?.metaDescription || '',
    publish: currentSoFi?.publish || true,
    comments: true,
  }), [currentSoFi]);

  const methods = useForm<SoFiCreateSchemaType>({
    mode: 'all',
    resolver: zodResolver(SoFiCreateSchema),
    defaultValues,
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      reset();
      showPreview.onFalse();
      toast.success(currentSoFi ? 'Atualizado com sucesso!' : 'SoFi criado com sucesso!');
      router.push(paths.dashboard.sofi.root);
    ...
    <Card>
      ...
      <Collapse in={openDetails.value}>
        ...
        <Stack spacing={3} sx={{ p: 3 }}>
          <RHFTextField name="title" label="Título do SoFi" placeholder="Ex: Inovação em Paraty" />
          ...
          <Stack spacing={1.5}>
            <Typography variant="subtitle2">Capa do SoFi</Typography>
  ...
  const renderActions = () => (
    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 2 }}>
      ...
      <Button
        ...
        disabled={!isValid}
      >
        {!currentSoFi ? 'Criar SoFiagem' : 'Salvar Alterações'}
      </Button>
    </Box>
  );
