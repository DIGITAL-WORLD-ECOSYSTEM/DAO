 ...
 * Role: SoFi List View (Client-Side Filtering & Search)
...
import { useGetSoFis } from 'src/actions/blog';

import { Label } from 'src/components/label';
import { Iconify } from 'src/components/iconify';
import { CustomBreadcrumbs } from 'src/components/custom-breadcrumbs';

import { SoFiSort } from '../components/post-sort';
import { SoFiSearch } from '../components/post-search';
import { SoFiListHorizontal } from '../item/list-horizontal';
...
export function SoFiListView({ sofi: initialSoFis }: Props) {
  const theme = useTheme();
  const { sofi: fetchedSoFis, sofiLoading } = useGetSoFis();
  
  const sofi = useMemo(() => initialSoFis || fetchedSoFis || [], [initialSoFis, fetchedSoFis]);
  ...
  const publishCounts = useMemo(
    () => ({
      all: sofi.length,
      published: sofi.filter((post) => post.publish).length,
      draft: sofi.filter((post) => !post.publish).length,
    }),
    [sofi]
  );

  const searchResults = useMemo(() => {
    if (!searchQuery) return sofi;
    return sofi.filter((post) =>
      ...
      >
        ...
        >
          <SoFiSearch
            ...
            redirectPath={(title: string) => paths.dashboard.sofi.details(title)}
          />

          <SoFiSort sort={sortBy} onSort={setSortBy} sortOptions={POST_SORT_OPTIONS} />
        </Box>
        ...
        <SoFiListHorizontal 
          sofi={dataFiltered} 
          loading={sofiLoading && sofi.length === 0} 
        />
      </Box>
    </BlogLayout>
  );
