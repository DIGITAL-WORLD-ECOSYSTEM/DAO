...
import type { ISoFiItem, ISoFiComment } from 'src/types/blog';
...
import { SoFiItem } from '../../item/item';
import { SoFiCommentForm } from '../../forms/post-comment-form';
import { SoFiCommentList } from '../../details/post-comment-list';
import { SoFiDetailsHero } from '../../details/post-details-hero';
...
export function SoFiDetailsHomeView({ post, latestSoFis }: Props) {
  ...
  const comments: ISoFiComment[] = post?.comments ?? [];

  const recentSoFis = useMemo(() => {
    const latest = latestSoFis ?? [];
    ...
    <>
      ...
      <Container maxWidth={false}>
        <Stack sx={{ maxWidth: 720, mx: 'auto' }}>
          ...
          <SoFiCommentList comments={comments} />
        </Stack>
      </Container>
      ...
      {!!recentSoFis.length && (
        <Container sx={{ pb: 15 }}>
          ...
          <Box sx={gridStyles}>
            {recentSoFis.map((latestSoFi) => (
              <SoFiItem
                key={latestSoFi.id}
                post={latestSoFi}
                detailsHref={paths.post.details(latestSoFi.title)}
              />
            ))}
          </Box>
        </Container>
