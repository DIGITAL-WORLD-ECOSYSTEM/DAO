...
path = 'src/sections/account/account-socials.tsx'
with open(path, 'r') as f:
    content = f.read()

# Fix qualityBadge typing
old_badge_def = "let qualityBadge = { label: 'Iniciante', color: 'error' as const };"
new_badge_def = "let qualityBadge: { label: string, color: 'default' | 'primary' | 'secondary' | 'error' | 'info' | 'success' | 'warning' } = { label: 'Iniciante', color: 'error' };"
content = content.replace(old_badge_def, new_badge_def)

# Remove 'as const' from the assignments to avoid further issues, as the type is now explicit
content = content.replace("color: 'primary' as const", "color: 'primary'")
content = content.replace("color: 'success' as const", "color: 'success'")
content = content.replace("color: 'warning' as const", "color: 'warning'")
