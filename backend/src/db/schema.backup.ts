...
export const users = sqliteTable(
	'users',
	{
		...
		role: text('role', { enum: ['citizen', 'partner', 'admin', 'system', 'dev', 'user'] }).default('citizen'),
...
export const rePropertyWorkflow = sqliteTable(
	're_property_workflow',
	{
		...
		propertyId: integer('property_id')
			.notNull()
			.references(() => reProperties.id, { onDelete: 'cascade' }),
		actorCitizenId: integer('actor_citizen_id').references(() => citizens.id, { onDelete: 'set null' }),
...
export const bounties = sqliteTable(
	'bounties',
	{
		...
		status: text('status', { enum: ['open', 'assigned', 'review', 'completed', 'cancelled'] }).default('open'),
...
export const integrationConfigs = sqliteTable(
	...
	(table) => ({
		providerEnvIdx: uniqueIndex('idx_integration_provider_env').on(table.provider, table.environment),
	}),
);
...
export const integrationSecrets = sqliteTable(
	...
	(table) => ({
		configKeyIdx: uniqueIndex('idx_integration_secret_config_key').on(table.configId, table.keyName),
	}),
);

export const integrationSecretVersions = sqliteTable(
	'integration_secret_versions',
	{
		id: text('id').primaryKey(), // UUID
		secretId: text('secret_id').notNull().references(() => integrationSecrets.id, { onDelete: 'cascade' }),
...
export const auditLogsImmutable = sqliteTable(
...
export const emailAccounts = sqliteTable('email_accounts', {
	...
	type: text('type', { enum: ['Atendimento', 'Financeiro', 'Juridico', 'Sistema', 'Marketing', 'Governança', 'Newsletter', 'Diretoria'] }),
	criticality: text('criticality', { enum: ['Baixa', 'Média', 'Alta', 'Crítica'] }),
	providerInbound: text('provider_inbound').default('zoho'),
	providerOutbound: text('provider_outbound').default('resend'),
	imapHost: text('imap_host'),
	imapPort: integer('imap_port'),
	oauthToken: text('oauth_token'),
	appPassword: text('app_password'), // encrypted
	signatureHtml: text('signature_html'),
	replyTo: text('reply_to'),
	color: text('color'),
	status: text('status', { enum: ['Provisionando', 'Ativa', 'Sincronizando', 'Erro', 'Suspensa', 'Arquivada', 'Desativada'] }).default('Provisionando'),
	healthStatus: text('health_status', { enum: ['Verde', 'Amarelo', 'Vermelho', 'Cinza'] }).default('Cinza'),
...
export const emails = sqliteTable('emails', {
	...
	folderId: text('folder_id').references(() => emailFolders.id),
	...
	folderIdIdx: index('idx_emails_folder_id').on(table.folderId),
...
export const emailMessageLabels = sqliteTable('email_message_labels', {
	messageId: text('message_id').references(() => emails.id).notNull(),
	labelId: text('label_id').references(() => emailLabels.id).notNull(),
}, (t) => ({
...
export const emailSyncJobs = sqliteTable('email_sync_jobs', {
	...
	status: text('status', { enum: ['Running', 'Success', 'Failed', 'Timeout'] }).default('Running'),
