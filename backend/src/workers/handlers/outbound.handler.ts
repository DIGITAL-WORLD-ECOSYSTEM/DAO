...
export class OutboundQueueHandler {
  ...
  async execute(envelope: QueueEnvelope<OutboundEmailQueueDTO>) {
    ...
    if (emailRecord.status === 'sent' || emailRecord.status === 'delivered') {
      console.log(
        `[OutboundQueueHandler] Email ${emailId} is already ${emailRecord.status}. Skipping.`
      );
      return; // Idempotency
    }
    ...
    const result = await resendProvider.send({
      ...
      cc: (emailRecord.cc
        ? emailRecord.cc.split(',').map((c) => ({ address: c.trim() }))
        : undefined) as any,
      bcc: (emailRecord.bcc
        ? emailRecord.bcc.split(',').map((b) => ({ address: b.trim() }))
        : undefined) as any,
