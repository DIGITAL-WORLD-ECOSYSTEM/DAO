...
export interface StorageProvider {
	 ...
	 */
	upload(filename: string, content: Buffer | Uint8Array, mimeType: string): Promise<{ key: string; publicUrl?: string }>;
...
export interface SpamProvider {
	 ...
	 */
	validate(headers: Record<string, string>, authMetadata: Record<string, any>): Promise<{ isSafe: boolean; score: number; reasons: string[] }>;
}
