...
export class AttachmentService {
	...
	async processAttachments(attachments: NormalizeAttachmentDTO[]): Promise<NormalizeAttachmentDTO[]> {
		if (!attachments || attachments.length === 0) return [];
		
		const processed: NormalizeAttachmentDTO[] = [];

		for (const att of attachments) {
			const buffer = Buffer.isBuffer(att.content) ? att.content : 
						   (att.content instanceof Uint8Array ? Buffer.from(att.content) : Buffer.from(att.content, 'base64'));
