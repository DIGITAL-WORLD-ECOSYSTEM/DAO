...
export class AuditService {
  ...
  async log(event: AuditEvent): Promise<void> {
    ...
    if (event.status === 'success' && this.kv) {
      ...
      if (event.metrics?.dbWrites)
        tasks.push(this.incrementKV('stats:db_writes_24h', event.metrics.dbWrites));
      if (event.metrics?.dbReads)
        tasks.push(this.incrementKV('stats:db_reads_24h', event.metrics.dbReads));
