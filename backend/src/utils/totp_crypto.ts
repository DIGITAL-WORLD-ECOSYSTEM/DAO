...
export async function encryptTotpSecret(secret: string, jwtSecret: string): Promise<string> {
  ...
  const ciphertextHex = Array.from(ciphertextBytes)
    .map((b) => b.toString(16).padStart(2, '0'))
    .join('');
  const authTagHex = Array.from(authTagBytes)
    .map((b) => b.toString(16).padStart(2, '0'))
    .join('');
...
export async function decryptTotpSecret(encryptedJson: string, jwtSecret: string): Promise<string> {
  ...
  const ciphertext = new Uint8Array(
    parsed.ciphertext.match(/.{1,2}/g).map((byte: string) => parseInt(byte, 16))
  );
  const authTag = new Uint8Array(
    parsed.authTag.match(/.{1,2}/g).map((byte: string) => parseInt(byte, 16))
  );
  ...
  const decryptedBuffer = await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, aesKey, combined);
