...
export class CryptoVault {
	...
	static async decrypt(encryptedPayload: string, masterSecret: string): Promise<string> {
		...
		try {
			...
			throw new Error('Falha catastrófica ao descriptografar segredo. MASTER_SECRET inválida ou payload violado.');
		}
	}
	 ...
	 */
	static async rotateMasterKey(encryptedPayload: string, oldMasterSecret: string, newMasterSecret: string): Promise<string> {
	 ...
	 */
	static async generateEventHash(payload: Record<string, any>, previousHash: string = 'GENESIS'): Promise<string> {
