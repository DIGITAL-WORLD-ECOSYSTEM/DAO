...
export function parseSshEd25519PublicKey(sshKeyStr: string): Uint8Array {
	...
	const keyLen = (bytes[keyLenOffset] << 24) | (bytes[keyLenOffset + 1] << 16) | (bytes[keyLenOffset + 2] << 8) | bytes[keyLenOffset + 3];
...
export async function verifySshEd25519Signature(
	...
	try {
		...
		if (sigBytes.length > 64) {
			...
			const sigLen = (sigBytes[sigLenOffset] << 24) | (sigBytes[sigLenOffset + 1] << 16) | (sigBytes[sigLenOffset + 2] << 8) | sigBytes[sigLenOffset + 3];
