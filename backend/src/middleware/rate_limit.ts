...
export interface RateLimiterProvider {
	isAllowed(ip: string, config: RateLimitConfig): Promise<{ allowed: boolean, retryAfter?: number }>;
}
...
class KVProvider implements RateLimiterProvider {
	...
	async isAllowed(ip: string, config: RateLimitConfig) {
		...
		// TTL for KV
		await this.kv.put(key, JSON.stringify(record), { expirationTtl: Math.ceil(config.windowMs / 1000) });
		return { allowed: true };
	}
}
