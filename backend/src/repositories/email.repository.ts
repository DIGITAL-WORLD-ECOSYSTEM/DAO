...
export class EmailRepository {
	...
	async createInboundEmail(data: { messageId: string; accountId: string; sender: string; recipient: string; subject: string; bodyHtml: string; bodyText: string; createdAt: Date; }) {
		const id = crypto.randomUUID();
		await this.db.insert(emails).values({
			id,
