...
app.get('/binance/price/:symbol', async (c) => {
  ...
  try {
    ...
    if (!res.ok) {
      return c.json(
        { success: false, message: 'Falha ao conectar na Binance', error: await res.text() },
        502
      );
    }
...
app.get('/binance/balance', async (c) => {
  ...
  try {
    ...
    if (!res.ok) {
      return c.json(
        { success: false, message: 'Binance API Error', error: await res.text() },
        res.status as any
      );
    }
    ...
    const nonZeroBalances =
      data.balances?.filter((b: any) => parseFloat(b.free) > 0 || parseFloat(b.locked) > 0) || [];
