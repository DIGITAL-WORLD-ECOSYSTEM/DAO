...
blog.get('/', async (c) => {
  ...
  try {
    ...
    return c.json(
      { success: false, message: `Erro ao buscar feed: ${error.message || 'Erro desconhecido'}` },
      500
    );
  }
});
...
blog.get('/search', async (c) => {
  ...
  try {
    ...
    return c.json(
      { success: false, message: `Erro na busca: ${error.message || 'Erro desconhecido'}` },
      500
    );
  }
});
...
blog.get('/:slug', async (c) => {
  ...
  try {
    ...
    return c.json(
      {
        success: false,
        message: `Erro ao carregar artigo: ${error.message || 'Erro desconhecido'}`,
      },
      500
    );
  }
      ...
      return c.json(
        {
          ...
          errors: (result.error as any).flatten(),
        },
        400
      );
    }
  ...
  async (c) => {
    ...
    try {
      ...
      return c.json(
        {
          success: false,
          message: `Erro interno do servidor: ${error.message || 'Não foi possível criar o post.'}`,
        },
        500
      );
    }
...
blog.post(
  ...
  zValidator('param', postIdSchema, (result, c) => {
    if (!result.success) {
      return c.json(
        { success: false, message: 'ID de post inválido', errors: (result.error as any).flatten() },
        400
      );
    }
  }),
  ...
  zValidator('param', postIdSchema, (result, c) => {
    if (!result.success) {
      return c.json(
        {
          success: false,
          message: 'ID de post inválido no parâmetro',
          errors: (result.error as any).flatten(),
        },
        400
      );
  ...
  zValidator('json', createPostSchema, (result, c) => {
    if (!result.success) {
      return c.json(
        {
          success: false,
          message: 'Erro de validação no payload de atualização',
          errors: (result.error as any).flatten(),
        },
        400
      );
  ...
  async (c) => {
    ...
    try {
      ...
      const [existingPost] = await db
        .select({ authorId: posts.authorId, coverUrl: posts.coverUrl })
        .from(posts)
        .where(eq(posts.id, id));
      if (!existingPost) return c.json({ success: false, message: 'Post não encontrado' }, 404);
      if (existingPost.authorId !== user.userId)
        return c.json({ success: false, message: 'Sem permissão para editar este post.' }, 403);
      ...
      if (
        postData.coverUrl &&
        existingPost.coverUrl &&
        postData.coverUrl !== existingPost.coverUrl
      ) {
...
blog.delete('/:id', zValidator('param', postIdSchema), async (c) => {
  ...
  try {
    ...
    if (!existingPost) return c.json({ success: false, message: 'Post não encontrado' }, 404);
    if (existingPost.authorId !== user.userId)
      return c.json({ success: false, message: 'Sem permissão para deletar este post.' }, 403);
