...
treasury.get('/citizen/:id/ledger', async (c) => {
	...
	try {
		// 1. Fetch Citizen
		const citizenResult = await db.select().from(citizens).where(eq(citizens.id, citizenId)).limit(1);
		...
		if (citizen.userId) {
			const contractResult = await db.select().from(contracts).where(eq(contracts.userId, citizen.userId)).limit(1);
		...
		const txs = await db.select().from(treasuryLedger).where(eq(treasuryLedger.citizenId, citizenId)).orderBy(desc(treasuryLedger.createdAt));
		...
		// 1. Associate Data
		const fullName = `${citizen.firstName || ''} ${citizen.lastName || ''}`.trim() || citizen.username;
		...
		const associateData = {
			...
			status: citizen.status === 'active' ? 'active' : citizen.status === 'suspended' ? 'suspended' : 'inactive',
		};
		...
		const paidTxs = processedTransactions.filter((t: any) => t.direction === 'inbound' && t.status === 'confirmed');
		...
		const obligationsRes = {
			...
			pendingInstallments: contractData ? (contractData.totalInstallments || 0) - (contractData.paidInstallments || 0) : 0,
