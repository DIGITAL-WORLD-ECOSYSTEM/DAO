...
router.get('/tables', async (c) => {
	try {
		...
		if (!db) {
			return error(c, 'Cloudflare D1 binding (DB) not found in context environment.', 'D1_NOT_FOUND', 500);
		}
		...
		for (const tableName of tablesList) {
			try {
				...
				else if (tableName.includes('integration') || tableName.includes('audit')) domain = 'PLATFORM';
				else domain = 'APP';
