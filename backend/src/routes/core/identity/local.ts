...
const loginRateLimiter = async (c: any, next: any) => {
	...
	if (count >= limit) {
		return c.json({ success: false, message: 'Número de tentativas de login excedido. Tente novamente em 10 minutos.' }, 429);
	}
...
localAuth.post('/register', zValidator('json', signUpSchema), async (c) => {
	...
	try {
		...
		const userRole = newUser.email === 'dev@asppibra.com' ? 'dev' : (newUser.role === 'citizen' ? 'user' : (newUser.role || 'user'));
...
localAuth.post('/login', loginRateLimiter, zValidator('json', legacyLoginSchema), async (c) => {
	...
	try {
		...
		const { issueSession } = await import('../../../utils/auth');
		const userRole = user.email === 'dev@asppibra.com' ? 'dev' : (user.role === 'citizen' ? 'user' : (user.role || 'user'));
