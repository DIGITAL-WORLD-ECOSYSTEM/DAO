...
function encodeSshPublicKey(pubKeyBytes: Uint8Array): string {
	const header = new Uint8Array([
		0, 0, 0, 11, ...new TextEncoder().encode('ssh-ed25519'),
		0, 0, 0, 32
	]);
...
describe('Developer SSH Verification Flow & Secure Headers', () => {
	...
	beforeEach(async () => {
		...
		const pubRaw = new Uint8Array((await crypto.subtle.exportKey('raw', keyPair.publicKey)) as ArrayBuffer);
	...
	it('GET /developer/challenge returns 200 and challenge UUID for developer', async () => {
		...
		expect(mockKv.put).toHaveBeenCalledWith('developer_challenge:99', expect.any(String), { expirationTtl: 300 });
	});
