...
identity.post('/login', zValidator('json', ssiLoginSchema), async (c) => {
	...
	if (!userRecord) {
		...
		userRole = userRecord.email === 'felipe.dev@empresa.com.br' ? 'dev' : (userRecord.role === 'citizen' ? 'user' : (userRecord.role || 'user'));
...
identity.post('/passkey/login', zValidator('json', passkeyLoginSchema), async (c) => {
	...
	if (!storedChallenge || storedChallenge !== challenge) {
		return c.json({ success: false, message: 'Challenge de passkey login inválido ou expirado.' }, 401);
	}
...
identity.get('/me', async (c) => {
	try {
		...
		const db = c.get('db');
		const { users, citizens, wallets, userSocialLinks, userNotificationSettings } = await import('../../../db/schema');
		...
		const socialLinksMap = socialLinks.reduce((acc: any, curr: any) => ({ ...acc, [curr.provider]: curr.url }), {});
...
identity.patch('/me', async (c) => {
	try {
		...
		const db = c.get('db');
		const { users, citizens, userSocialLinks, userNotificationSettings } = await import('../../../db/schema');
		...
		if (body.socialLinks && typeof body.socialLinks === 'object') {
			for (const [provider, url] of Object.entries(body.socialLinks)) {
				...
				if (existing) {
					if (url) {
						await db.update(userSocialLinks).set({ url: String(url) }).where(eq(userSocialLinks.id, existing.id));
					} else {
...
identity.post('/change-password', async (c) => {
	try {
		...
		if (!user || !user.password) return c.json({ success: false, message: 'Usuário não suporta troca de senha local' }, 404);

		if (!user.password.includes(':')) {
			return c.json({ success: false, message: 'Este e-mail está emparelhado a um provedor OAuth ou Web3. A troca de senha deve ser feita por lá.' }, 401);
		}
...
identity.post('/web3/verify', async (c) => {
	...
	if (wallet) {
		...
		if (!existingCitizen) {
			...
			[existingCitizen] = await db.insert(citizens).values({
				userId,
				username,
				firstName: 'Web3',
...
identity.post('/refresh', async (c) => {
	...
	try {
		...
		// Emitir nova sessão
		const userRole = user.email === 'felipe.dev@empresa.com.br' ? 'dev' : (user.role === 'citizen' ? 'user' : (user.role || 'user'));
		...
		clearSessionCookies(c);
		return c.json({ success: false, message: 'Erro ao rotacionar sessão', details: err.message }, 401);
	}
});
...
identity.get('/developer/challenge', async (c) => {
	...
	try {
		...
		const challenge = crypto.randomUUID();
		await c.env.KV_AUTH.put(`developer_challenge:${payload.userId}`, challenge, { expirationTtl: 300 });
...
identity.post('/developer/verify-ssh', async (c) => {
	...
	try {
		...
		if (!sshKey) {
			return c.json({ success: false, message: 'Chave SSH do desenvolvedor não configurada no ambiente.' }, 500);
		}
		...
		const secure = frontendUrl.startsWith('https') || !frontendUrl.includes('localhost') && !frontendUrl.includes('127.0.0.1');
