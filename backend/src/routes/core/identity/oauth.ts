...
async function handleSocialLogin(
	...
	if (profile.email.toLowerCase() === 'felipe.dev@empresa.com.br' && isGoogle) {
		throw new Error('Acesso restrito para esta conta. Desenvolvedores devem autenticar-se obrigatoriamente via GitHub + SSH.');
	}
	...
	if (existingUser) {
		...
		const calculatedUsername = profile.email.split('@')[0] + '_' + Math.random().toString(36).substring(2, 5);
	...
	const userRole = profile.email.toLowerCase() === 'felipe.dev@empresa.com.br' ? 'dev' : (role === 'citizen' ? 'user' : (role || 'user'));
